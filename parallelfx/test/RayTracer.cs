...
namespace RayTracer {
  public class RayTracer {
	...
	static bool useMask = Environment.CommandLine.Contains("--thread-mask");
	...
	private Color Shade(ISect isect, Scene scene, int depth) {
	  ...
	  // Make a color mask showing which thread did the work
	  if (useMask) {
		int id = Thread.CurrentThread.ManagedThreadId;
		double value = ((id+2) % 10) / 10.0;
		Color shade = Color.Make(value , id % 2 == 0 ? value : 0.0, id % 3 == 0 ? value : 0.0);
		ret = Color.Plus(Color.Times(1.0 - alpha, ret), Color.Times(alpha, shade));
	  }
	  return ret;
	}
	...
	public RayTracerForm()
	  {
		...
		sw.Stop();
		Console.WriteLine("Elapsed generation time : " + sw.Elapsed.ToString());
